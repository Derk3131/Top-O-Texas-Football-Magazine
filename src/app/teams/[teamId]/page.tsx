    ...
    <main className="flex flex-col items-center min-h-screen p-6 md:p-12 lg:p-24 bg-gray-100 text-black">
      <div className="w-full max-w-4xl bg-white p-8 rounded-lg shadow-lg">
        <div className="flex flex-col sm:flex-row items-center mb-8">
          <img src={team.logo || '/images/default-team-logo.png'} alt={`${team.name} Logo`} className="w-32 h-32 sm:w-40 sm:h-40 object-contain mb-4 sm:mb-0 sm:mr-8 rounded-full border-2 border-red-600"/>
        ...
        <section className="mb-8">
          <h2 className="text-2xl font-bold mb-4 font-oswald border-b-2 border-red-600 pb-2">Coach Submitted Content</h2>
          <p className="text-gray-700 font-open-sans italic">{team.coachNotes || 'No coach notes available at this time.'}</p>
        </section>

        <section className="mb-8">
          <h2 className="text-2xl font-bold mb-4 font-oswald border-b-2 border-red-600 pb-2">Photos</h2>
          {team.photos && team.photos.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {team.photos.map((photo, index) => (
                <img key={index} src={photo} alt={`${team.name} photo ${index + 1}`} className="w-full h-auto object-cover rounded shadow-md"/>
              ))}
            </div>
          ) : (
            <p className="text-gray-600 font-open-sans">Team photos coming soon.</p>
          )}
        </section>

        <section>
          <h2 className="text-2xl font-bold mb-4 font-oswald border-b-2 border-red-600 pb-2">Videos</h2>
          {team.videos && team.videos.length > 0 ? (
            ...
            <p className="text-gray-600 font-open-sans">Team videos coming soon. (Placeholder for HUDL/YouTube highlights)</p>
          )}
        </section>
